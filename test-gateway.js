// Test suite to verify all required API gateway features and contracts

process.env.USE_MOCK = 'true';
process.env.PORT = '4000';

const http = require('http');
const server = require('./server'); // Starts the server on port 4000
...
async function runTests() {
  console.log('\n--- Starting API Gateway Contract Validation Tests ---\n');
  ...
  try {
    ...
    console.log('🎉 ALL TESTS PASSED SUCCESSFULLY! Gateway matches contract specification.');
    process.exit(0);
  } catch (err) {
...
// Wait for connection initialization before running tests
